// sp-views.jsx — Sidebar, HomeView, ArticleView. -> window
const { useState: useStateV, useMemo } = React;

function Sidebar({ route, onNavigate, onSubscribeAll, collapsed, onToggle }) {
  const { PRIMER, TOPICS, relTime, daysAgo } = window.SP_DATA;
...
function HomeView({ onNavigate, onSubscribe, onSubscribeAll }) {
  const { recent, TOPICS, PRIMER, fmtDate, relTime } = window.SP_DATA;
...
function ArticleView({ page, onNavigate, onSubscribe }) {
  const { fmtDate, relTime, byId } = window.SP_DATA;
  ...
  return (
    <div style={{ maxWidth: 740, margin: "0 auto", padding: "clamp(28px, 5vw, 60px) clamp(20px, 5vw, 56px) 48px" }}>
      ...
      {/* meta row: last updated + subscribe */}
...
Object.assign(window, { Sidebar, HomeView, ArticleView });
