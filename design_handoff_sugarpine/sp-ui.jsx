// sp-ui.jsx — small presentational primitives + subscribe modal. -> window
const { useState, useEffect, useRef } = React;

// Minimal sugar-pine mark: three stacked triangles + a trunk. Simple shapes only.
function PineMark({ size = 26, color = "var(--accent)" }) {
  return (
...
// recency dot: bright if updated within 7 days, dim otherwise
function RecencyDot({ updated, withLabel }) {
  const { daysAgo, relTime } = window.SP_DATA;
...
function SubscribeModal({ open, target, onClose }) {
  ...
  useEffect(() => {
    if (open) { setDone(false); setEmail(""); setFreq("instant"); setTimeout(() => inputRef.current && inputRef.current.focus(), 60); }
  }, [open, target]);
  ...
  return (
    <div onMouseDown={onClose} style={{
      ...
      background: "color-mix(in srgb, var(--bg) 62%, transparent)", backdropFilter: "blur(6px)",
      padding: 24,
    }}>
      <div onMouseDown={(e) => e.stopPropagation()} style={{
        width: "min(460px, 100%)", background: "var(--bg-card)", border: "1px solid var(--border)",
        borderRadius: 16, padding: "30px 30px 26px", boxShadow: "0 30px 80px -20px rgba(0,0,0,.6)",
        position: "relative",
      }}>
        ...
        {!done ? (
          <React.Fragment>
            ...
            <button disabled={!valid} onClick={() => setDone(true)} style={{
              width: "100%", padding: "13px", borderRadius: 10, border: "none", cursor: valid ? "pointer" : "not-allowed",
              background: valid ? "var(--accent)" : "var(--border)", color: valid ? "var(--accent-on)" : "var(--text-faint)",
          ...
          <div style={{ textAlign: "center", padding: "12px 0 4px" }}>
            ...
            <p style={{ margin: "0 0 22px", color: "var(--text-dim)", fontSize: 14.5, lineHeight: 1.6 }}>
              We'll email <span style={{ color: "var(--text)" }}>{email}</span> {freq === "instant" ? "as soon as" : "in a weekly digest when"} {whole ? "anything on the site" : target.title} {freq === "instant" ? "changes" : "changes"}.
            </p>
...
// lite youtube embed: thumbnail + play, swaps to iframe on click
...
Object.assign(window, { PineMark, Eyebrow, RecencyDot, SubscribeModal, YouTubeEmbed });
