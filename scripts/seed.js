...
// For prod: node scripts/seed.js --prod  (requires .env.prod with prod keys)
...
// ── Page data ─────────────────────────────────────────────────────────────────
// id is omitted — auto-generated by the database
// slug is the URL-safe identifier, derived from the page title
// position controls display order (global, ascending)
// related uses slugs — resolved to integer IDs at seed time
...
const PAGES = [
  ...
  {
    slug: "text-generation", group: "Living Landscape", title: "Text Generation", position: 3,
    ...
    related: ["coding", "benchmarks", "ai-glossary"],
  },
  {
    slug: "image-and-video", group: "Living Landscape", title: "Image & Video", position: 4,
    ...
    related: ["deepfakes-and-authenticity", "text-generation"],
  },
  {
    slug: "deepfakes-and-authenticity", group: "Living Landscape", title: "Deepfakes & Authenticity", position: 5,
    ...
    related: ["image-and-video", "what-is-ai"],
  },
  {
    slug: "ai-in-science", group: "Living Landscape", title: "AI in Science", position: 6,
    ...
    related: ["mathematics", "benchmarks"],
  },
  {
    slug: "mathematics", group: "Living Landscape", title: "Mathematics", position: 7,
    ...
    related: ["ai-in-science", "benchmarks"],
  },
  {
    slug: "coding", group: "Living Landscape", title: "Coding", position: 8,
    ...
    related: ["text-generation", "builders-frameworks", "the-anthropic-stack"],
  },
  {
    slug: "robotics", group: "Living Landscape", title: "Robotics", position: 9,
    ...
    related: ["ai-in-science", "what-is-ai"],
  },
  {
    slug: "the-anthropic-stack", group: "Living Landscape", title: "The Anthropic Stack", position: 10,
    ...
    related: ["coding", "ai-glossary", "builders-frameworks"],
  },
  {
    slug: "agent-platforms", group: "Living Landscape", title: "Agent Platforms", position: 11,
    ...
    related: ["the-anthropic-stack", "builders-frameworks", "signal-vs-hype"],
  },
  {
    slug: "builders-frameworks", group: "Living Landscape", title: "Builder's Frameworks", position: 12,
    ...
    related: ["the-anthropic-stack", "agent-platforms", "coding"],
  },
  {
    slug: "signal-vs-hype", group: "Living Landscape", title: "Signal vs. Hype", position: 13,
    ...
    related: ["agent-platforms", "builders-frameworks", "benchmarks"],
  },
  {
    slug: "benchmarks", group: "Living Landscape", title: "Benchmarks", position: 14,
    ...
    related: ["text-generation", "mathematics", "signal-vs-hype"],
  },
]
...
async function seed() {
  // Step 1: Upsert all pages (conflict on slug)
  ...
  // Step 2: Fetch all pages to build slug → integer id map
  ...
  // Step 3: Upsert sections, cards, and related for each page
  for (const page of PAGES) {
    const pageId = slugToId[page.slug]
    if (!pageId) { console.error('No ID found for slug:', page.slug); continue }

    if (page.sections?.length) {
      await supabase.from('page_sections').delete().eq('page_id', pageId)
      const { error } = await supabase.from('page_sections')
        .insert(page.sections.map(s => ({ ...s, page_id: pageId })))
    ...
    if (page.cards?.length) {
      ...
      const { error } = await supabase.from('page_cards')
        .insert(page.cards.map(c => ({ ...c, page_id: pageId })))
    ...
    if (page.related?.length) {
      ...
      const relatedRows = page.related
        .map(relSlug => ({ page_id: pageId, related_id: slugToId[relSlug] }))
        .filter(r => r.related_id)
      const { error } = await supabase.from('page_related').insert(relatedRows)
